...
def extract_skull(ct_img_path: str) -> str:
    """
    ...
    skull_mask_img = sitk.Image(ct_img.GetWidth(), ct_img.GetHeight(), ct_img.GetDepth(), sitk.sitkFloat32)
    ...
    print('Name of the output skull image: ', output_ct_image_name)
...
def normalize_ct_scan(ct_nda: np.array) -> np.array:
...
def otsu_thresholding(ct_normalized_nda: np.array) -> np.array:
...
def get_2_maximum_2d_regions(max_binary: np.array):
...
def get_1_maximum_2d_region(max_second_binary: np.array):
...
def image_opening_2d(max_second_binary: np.array, structure: np.array = np.ones((15, 15))):
    ...
    for i in range(max_second_binary.shape[0]):
        new_max_second_binary[i,:,:] = ndimage.binary_opening(max_second_binary[i,:,:].astype(int),
                                                              structure=structure).astype(np.float32)

    return new_max_second_binary


def remove_ct_scan_device(ct_img_path: str) -> str:
...
def contrast_stretch(ct_img_path: str, percent: tuple = (10,90)) -> str:
